...
const isGeocodeArgs = (args) => typeof args === 'object' && args !== null && typeof args.address === 'string';
const isReverseGeocodeArgs = (args) => typeof args === 'object' && args !== null && typeof args.lat === 'number' && typeof args.lng === 'number';
const isDirectionsArgs = (args) => typeof args === 'object' &&
    args !== null &&
    typeof args.origin === 'string' &&
    typeof args.destination === 'string' &&
    (args.mode === undefined || typeof args.mode === 'string');
const isDistanceMatrixArgs = (args) => typeof args === 'object' &&
    args !== null &&
    Array.isArray(args.origins) && args.origins.every((o) => typeof o === 'string') && // Add explicit any type
    Array.isArray(args.destinations) && args.destinations.every((d) => typeof d === 'string') && // Add explicit any type
    (args.mode === undefined || typeof args.mode === 'string');
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                switch (toolName) {
                    case 'geocode':
                        if (!isGeocodeArgs(args))
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for geocode');
                        response = await this.googleMapsClient.geocode({ params: { address: args.address, key: API_KEY } }); // Use non-null assertion
                        break;
                    case 'reverse_geocode':
                        if (!isReverseGeocodeArgs(args))
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for reverse_geocode');
                        ...
                        break;
                    case 'get_directions':
                        if (!isDirectionsArgs(args))
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for get_directions');
                        const directionsParams = {
                            origin: args.origin,
                            destination: args.destination,
                            mode: args.mode, // Cast needed as SDK type might be stricter
                    ...
                    case 'get_distance_matrix':
                        if (!isDistanceMatrixArgs(args))
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for get_distance_matrix');
                        const matrixParams = {
                            origins: args.origins,
                            destinations: args.destinations,
                            mode: args.mode, // Cast needed
                            key: API_KEY // Use non-null assertion
                        };
